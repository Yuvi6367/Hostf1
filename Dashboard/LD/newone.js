...
window.showPopup = async function (labourId, selectedDate = null) {
    ...
    <div class="row header">
        <div class="cell" data-i18="L&D.date">Date</div>
        <div class="cell" data-i18="L&D.appearance">Appearance</div>
        <div class="cell" data-i18="L&D.advance">Advance</div>
    </div>`;
...
async function renderAdvancesPopup(labourId, date) {
    ...
    try {
        ...
        attendance.advances.forEach((advance, index) => {
            ...
            <span data-i18="L&D.advance">Advance:</span> ${advance} 
            <button class="delete-advance" data-index="${index}" data-labour-id="${labourId}" data-date="${date}">🗑</button>
        `;
...
export async function updateCalculator(labourId) {
    ...
    try {
        ...
        // Update calculator UI with translation keys
        calculator.innerHTML = `
<div class="row">
    <div class="cell" data-i18n="L&D.balance">Balance:</div>
    <div class="cell">${balance.toFixed(2)}</div>
    <div class="cell" data-i18n="L&D.rate">Rate: ${ratePerDay}</div>
...
async function updateCalculatorForMonth(labourId, selectedDate) {
    ...
    <div class="row">
        ...
        <div class="cell">${balance.toFixed(2)}</div>
        <div class="cell" data-i18n="L&D.rate">Rate: ${ratePerDay}</div>
        <div class="cell" data-i18n="L&D.advance_money">Advance Money: ${advanceMoney.toFixed(2)}</div>
    </div>
    <hr class="separator">
    <div class="row">
        <div class="cell" data-i18n="L&D.total">Total:</div>
...
async function exportToCSV(labourId, selectedDate) {
    try {
        ...
        console.info("CSV exported successfully.");
    } catch (error) {
